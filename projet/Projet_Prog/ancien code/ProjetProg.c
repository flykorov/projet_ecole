...
int CHOIX_MODE()		//choix du mode de jeu
{
	clear();
	int mode;
	...
	printw("            |                 |       |                 |\n");
	printw("            |                 |       |                 |\n");
	printw("            |      Image      |       |     Chiffre     |\n");
	printw("            |                 |       |                 |\n");
	printw("            |_________________|       |_________________|\n");
	do
	{
		click_souris();
		if(L > 11 && L < 15 && C > 13 && C < 29)
		{
			mode = 1;
		}
		if(L > 11 && L < 15 && C > 40 && C < 57)
		{
			mode = 2;
		}
	}while(mode != 1 && mode != 2);
...
int CHOIX_DIF()		//choix de la difficulté
{
	clear();
	int dif;
	printw("                          _________________ \n");
...
int REJOUER()		//choix de continuer ou de s'arreter
{
	clear();
	int choix;
	printw("                          _________________ \n");
...
int** INI_TAB_BI()		//initialise les 2 tableaux bidimentionelle
{ 
	int i;
	int** MAT;
...
void CHARG_MAT_ORDER(int** ORDER_TAB)		//charge la matrice de réfernce
{
	int i, j;
	int v = 1;
	for(i=0; i<3; i++)
...
void CHARG_MAT_NUM(int** NUM_IN_GAME)		//charge une matrice aléatoire de chiffre différent compris entre 1 et 9
{
	int i, j, k, l, a, t;
	for(i=0; i<3; i++)
	{
		for(j=0; j<3; j++)
		{
			NUM_IN_GAME[i][j] = 0;		//initialisation à 0 pour éviter tout problème
		}
	}
	for(i=0; i<3; i++)
	{
		for(j=0; j<3; j++)
		{
			...
			do
			{
				t = 1;
				a = rand() % 9;
				a = a + 1;		//pour choisir des chiffres entre 1 et 9
				for(k=0; k<3; k++)
				{
					for(l=0; l<3; l++)	//vérifie si le chiffre est dans la matrice	
					{
			...
			}while(t == 0);		//recommence si le chiffre choisi est déjà dans la matrice
			NUM_IN_GAME[i][j] = a;		//affecte le chiffre qui à passer et réussi le test précédent
		}
	}			
}

void AFFICH_MAT_ORDER(int** ORDER_TAB)		//affiche la matrice de référence (pas utiliser dans le prog)
{
	int i, j;
	for(i=0; i<3; i++)
	{
...
void AFFICH_MAT_NUM(int** NUM_IN_GAME)		//affiche la matrice aléatoire (pas utiliser dans le prog)
{
	int i, j;
	for(i=0; i<3; i++)
	{
...
void LIB_MAT_ORDER(int** ORDER_TAB)		//libère l'éspace des matrices
{
	int i;
	for(i=0; i<3; i++)
	{
...
void LIB_MAT_NUM(int** NUM_IN_GAME)		//libère l'éspace des matrices
{
	int i;
	for(i=0; i<3; i++)
	{
...
int AFFICHE_CHIFFRE_MODE(int** NUM_IN_GAME)		//affiche les chiffres telles qu'il sont dans la matrice aléatoire
{
	int i, j, k, p, n;
	i = 0;
	for(k=0; k<3; k++)		//affiche en carré 3*3 les chiffres
	{
		for(j=0; j<3; j++)	//affiche la première ligne
		{
		...
		printw("\n");	
		for(j=0; j<3; j++)	//affiche la deuxième ligne
		{
			switch(NUM_IN_GAME[i][j])
			{
				case 1:
		...
		printw("\n");	
		for(j=0; j<3; j++)	//affiche la troisième ligne
		{
			switch(NUM_IN_GAME[i][j])
			{
				case 1:
		...
		printw("\n");	
		for(j=0; j<3; j++)	//affiche la quatrième ligne
		{
			switch(NUM_IN_GAME[i][j])
			{
				case 1:
		...
		printw("\n");	
		for(j=0; j<3; j++)	//affiche la cinquième ligne
		{
			switch(NUM_IN_GAME[i][j])
			{
				case 1:
		...
		i++;		//change la ligne de la matrice
		printw("\n");
	}
	printw("\n");printw("\n");printw("\n");
...
int CHIFFRE_MODE_FAC(int** ORDER_TAB, int** NUM_IN_GAME)
{
	...
	do
	{
		n = 0;		//valeur utilisé pour vérifié si l'utilisateur gagne ou pas 
		clear();
		...
		for(i=0; i<3; i++)		//vérifie si la matrice aléatoire modifié par le joueur est égal à la matrice de référence
		{
			for(j=0; j<3; j++)
			{
				if(ORDER_TAB[i][j] == NUM_IN_GAME[i][j])
				{
					n++;					//si n = 9 c'est gagner
				}
			}
		}			
	}while(n != 9);
...
int CHIFFRE_MODE_DIF(int** ORDER_TAB, int** NUM_IN_GAME)		//même chose avec une autre difficulté
{
	int g, i, j, n;
	do
	{
...
int main()
{
	...
	int choix;		//si l'utilisateur rejoue ou non
	int mode;		//qu'elle mode choisi par l'utilisateur
	int dif;		//qu'elle difficulté choisi par l'utilisateur
	int** ORDER_TAB;
	int** NUM_IN_GAME;
	...
	do
  	{
		CHARG_MAT_NUM(NUM_IN_GAME);
    		...
    		dif = CHOIX_DIF();
    		clear();
    		if(mode == 1 && dif == 1)		//redirige l'utilisateur vers le mode et la difficulté choisi
		{
			IMAGE_MODE_FAC();	//en travaux
		}
		if(mode == 1 && dif == 2)
		{	
			IMAGE_MODE_DIF();	//en travaux
		}
		if(mode == 2 && dif == 1)
		{
