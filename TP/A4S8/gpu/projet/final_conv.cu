/*
    ...
    Code acceleration GPU

    code crée et compiler sur linux (ubuntu)
    
    !!!  Ne fonctionne pas sur colab et windows !!!

    prendre le fichier ypinb pour faire fonctionner le code sur widnows/colab
...
// === Functions ===
__global__ void calculateHash(bool* indice, uint32_t size, char mot[1024][100], uint32_t* K, uint32_t* mdp)
{   
...
int main(int argc, char *argv[])
{
    ...
    while(continuer)
	{
        ...
        // 16, nb_mot/16
	 	...
	 	calculateHash<<<16, nb_mot/16>>>(device_indice, nb_case, ligne, K, mdp);
