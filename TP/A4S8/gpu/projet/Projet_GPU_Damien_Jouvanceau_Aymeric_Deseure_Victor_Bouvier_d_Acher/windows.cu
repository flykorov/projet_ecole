...
// === Functions ===
__global__ void calculateHash(uint32_t* res, uint32_t size, char mot[1024][100], uint32_t* K, int *verif)
{   
...
int main()
{
	...
	char nom[49] = "C:\\Users\\totod\\Downloads\\rockyou.txt\\rockyou.txt";
	...
	// cudaMemcpy(device_Hash, host_Hash, nb_mot * nb_case * sizeof(uint32_t), cudaMemcpyHostToDevice);

	// int nb_block = nb_mot / (nb_thread) + (nb_mot % (nb_thread) != 0);
	// dim3 grid(1, 1);
	// dim3 block(4, 1);
