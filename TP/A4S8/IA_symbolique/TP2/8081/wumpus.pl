:- module(wumpus, [updateEatWumpus/5]).
...
false_pit(Ls, ULs, NULs) :-
	interface_tab_etat(ULs, TLs, FLs),
	tab_false(Ls, TLs, FLs, NULs).

updateEatWumpus(Percept, Adjacents, Wall, Uncertain_Eternals, New_Uncertain_Eternals) :-
	trouver_case(Adjacents, Wall, Ls),
	

	(member(stench, Percept) ->
		true_pit(Ls, Uncertain_Eternals.eat_wumpus, NLs);
		false_pit(Ls, Uncertain_Eternals.eat_wumpus, NLs)
	),
