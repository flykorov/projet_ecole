...
void init_fifo() {
    ...
    pthread_key_create(&fifo.clef, NULL); // CHANGE
}
...
void put(struct fifo *f, int a) {
    ...
    long cpt = (long)pthread_getspecific(fifo.clef); // CHANGE
    pthread_setspecific(fifo.clef, (void*)cpt + 1); // CHANGE
...
void *prod(void* arg) {
    ...
    pthread_setspecific(fifo.clef, 0); // CHANGE
    ...
    printf("J'ai produis %ld fois\n", (long)pthread_getspecific(fifo.clef)); // CHANGE
    pthread_exit(NULL);
}
