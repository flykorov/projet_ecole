...
int prop_mat_virus(int x, int y, int size[x][y], int mat_pop[x][y], int rec_mat[x][y], int a, int b, int c, int u, int v)
{
	...
	if(u - 1 >= 0 && u - 1 < x && rec_mat[u-1][v] == 0 && mat_pop[u][v] == -1)
	{
	...
	if(v - 1 >= 0 && v - 1 < y && rec_mat[u][v-1] == 0 && mat_pop[u][v] == -1)
	{
	...
	if(u - 1 >= 0 && u - 1 < x && v - 1 >= 0 && v - 1 < y && rec_mat[u-1][v-1] == 0 && mat_pop[u][v] == -1)
	{
...
int aff_prop(int x, int y, int mat_pop[x][y])
{
	int i, j;
